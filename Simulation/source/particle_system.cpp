...
System::System(int amount_of_particles,
  ...
  internal_force_function = force;
  internal_potential_function = potential;
...
void System::update_system() {
  ...
  for(int i = 0; i < N-1; i++) {
    for(int j = i + 1; j < N; j++) {
      ...
      //Add particle forces
      F = (*internal_force_function)(
        particles[i] -> get_position(),
        particles[j] -> get_position());
...
void System::update_helper(int start, int end) {
...
void global_helper(System *p,int start, int end){
  p -> update_helper(start, end);
}
...
void System::update_system_threaded(unsigned int threads) {
  vector<thread> threadVect;
  unsigned int spread = N/threads + 1;
  unsigned int start = 0;
  unsigned int end = start + spread -1;
  for (unsigned int x = 0; x < threads; x++) {
    threadVect.emplace_back(global_helper, this,  start, end);
    start += spread;
    end += spread;
  }
...
void System::calc_potential() {
  ...
  for(int i = 0; i < N - 1; i++) {
    for(int j = i +1; j < N; j++) {
      potential_energy += (*internal_potential_function)(
        particles[i] -> get_position(),
        particles[j] -> get_position());
    }
  }
...
vector<double> System::get_potential_distribution() {
  ...
  for(int i = 0; i < N - 1; i++) {
    for(int j = i + 1; j < N; j++) {
      distribution.push_back((*internal_potential_function)(
        particles[i] -> get_position(),
        particles[j] -> get_position()));
    }
  }
...
vector<double> System::get_radius_distribution() {
  vector<double> distribution;
  Vector3d rad;
  double r;
  for(int i = 0; i < N - 1; i++) {
